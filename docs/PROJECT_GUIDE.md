...
│  - Prepend к input sequence                             │
└─────────────────────────────────────────────────────────┘
                           │
                           ▼
Input to Transformer: [mem0, mem1, ..., mem7, s1, s2, ..., sT]
                      └────── memory ──────┘ └── data ──┘

Идея: Memory slots = глобальный контекст для всей последовательности
      Модель может "записывать" в них паттерны и "читать" при предсказании
```

**Зачем Memory Bank?**
- Transformer с causal attention видит только прошлое
- Memory даёт доступ к "глобальному состоянию"
- Гипотеза: memory может хранить долгосрочные корреляции
...
### M0–M7: Каждый слот = одно Q3 условие

| Slot | Q3 условие | Физический смысл | Метрика для supervision |
|------|------------|------------------|-------------------------|
| **M0** | T0 Normalization | Фон/нормировка (архимедово) | `mean(spacings) ≈ 1.0` |
| **M1** | A1' Coverage | Покрытие режимов | `std / mean` (coefficient of variation) |
| **M2** | A2 Lipschitz | Стабильность к малым возмущениям | `max(|Δs|)` (max gradient) |
| **M3** | A3 Floor | Не схлопываться (level repulsion) | `min(spacings)` |
| **M4** | A3 Smoothness | Гладкость PSF (ω) | `std(|Δs|)` (gradient variance) |
| **M5** | Toeplitz | Устойчивость к дискретизации | `autocorr(lag=1)` |
| **M6** | RKHS Cap | Cap вкладов ("простые не кричат") | `max(spacings)` |
| **M7** | Spectral Gap | Устойчивый фокус (rollout) | `autocorr(lag=10)` |

### Как модель "узнаёт" Q3 инварианты?

**Вариант A: Semantic Supervision (aux_loss)**

Заставляем каждый слот предсказывать конкретную статистику:

```python
def compute_q3_invariants(x):  # x: [B, T] spacings
    m0 = x.mean(dim=1)                           # T0: mean ≈ 1.0
    m1 = x.std(dim=1) / (x.mean(dim=1) + 1e-8)   # A1': coverage
    m2 = (x[:,1:] - x[:,:-1]).abs().max(dim=1)   # A2: Lipschitz
    m3 = x.min(dim=1)                            # A3: floor
    m4 = (x[:,1:] - x[:,:-1]).abs().std(dim=1)   # A3: smoothness
    m5 = autocorr(x, lag=1)                      # Toeplitz
    m6 = x.max(dim=1)                            # RKHS: cap
    m7 = autocorr(x, lag=10)                     # Spectral gap
    return [m0, m1, m2, m3, m4, m5, m6, m7]
```

**Вариант B: Post-hoc Interpretation**

Обучаем без supervision, затем анализируем через ablation:
- Какой слот влияет на mean?
- Какой слот влияет на tail distribution?
- Какой слот стабилизирует rollout?
