...
GUE prediction (connected SFF in Heisenberg units):
- τ < 1: K(τ) = τ (ramp)
- τ > 1: K(τ) = 1 (plateau)

Reference: Haake, Quantum Signatures of Chaos; Mehta, Random Matrices
...
def compute_sff(unfolded_positions, tau_values):
    ...
    NOTE: We use the 2π convention where τ is dimensionless and τ=1 corresponds
    to the Heisenberg time. This differs from some files in this repo that omit
    the 2π factor (in which case τ=2π is the Heisenberg time).
...
def visualize_sff(results, save_path="sff_comparison.png"):
    ...
    ax.plot(tau[ramp_mask], results["sff_gue"][ramp_mask], 'g-', linewidth=3, label='GUE: K=τ', alpha=0.5)
...
def main():
    ...
    console.print("")
    console.print("[cyan]GUE prediction (connected SFF):[/]")
    console.print("  τ < 1: K(τ) = τ (linear ramp)")
