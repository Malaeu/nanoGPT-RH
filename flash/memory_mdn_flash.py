...
from mdn_flash import MDNHead, Block, MDNConfig, precompute_freqs_cis, apply_rotary_emb
...
class CausalSelfAttentionWithMemory(nn.Module):
    """
    Optimized Causal self-attention with RoPE and Flash Attention support.
    """
    ...
    def forward(self, x: torch.Tensor, freqs_cis: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if freqs_cis is not None:
            # Apply RoPE only to the sequence part
            q_mem, q_seq = q[:, :K], q[:, K:]
            k_mem, k_seq = k[:, :K], k[:, K:]
            q_seq, k_seq = apply_rotary_emb(q_seq, k_seq, freqs_cis[:T])
            q = torch.cat([q_mem, q_seq], dim=1)
            k = torch.cat([k_mem, k_seq], dim=1)
        ...
        # Custom mask for Memory + Causal Sequence
        # Memory tokens see all memory. Seq tokens see all memory + causal seq.
        mask = torch.ones(L, L, device=x.device, dtype=torch.bool)
        mask[K:, K:] = torch.tril(torch.ones(T, T, device=x.device, dtype=torch.bool))
        
        # Use high-performance scaled_dot_product_attention
        y = F.scaled_dot_product_attention(
            q, k, v,
            attn_mask=mask.view(1, 1, L, L),
            dropout_p=self.attn_dropout.p if self.training else 0.0,
            is_causal=False # Mask handled manually
        )
...
class BlockWithMemory(nn.Module):
    """Transformer block that handles memory prefix and RoPE."""
...
class MemoryMDN(nn.Module):
    """
    Transformer + MDN with Q3 Memory Bank, RoPE and Anchor Support.
    """
    ...
    def __init__(self, config: MemoryMDNConfig):
        ...
        self.register_buffer("freqs_cis", precompute_freqs_cis(config.n_embd // config.n_head, config.seq_len))
        ...
        print(f"MemoryMDN Flash: {n_params:,} parameters")
    ...
    def forward(
        ...
        """
        Args:
            spacings: (B, T) input spacings (usually residuals)
            targets: (B, T) target spacings for loss
            anchor: (B, T) analytical anchor (if any)
